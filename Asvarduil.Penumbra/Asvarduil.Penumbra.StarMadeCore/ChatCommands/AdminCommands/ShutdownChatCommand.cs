...
{
    public class ShutdownChatCommand : BaseAdminChatCommand
    {
        public override string Token => "!SHUTDOWN";
        public override string HelpText => "Usage: !SHUTDOWN [Duration].  Shuts the server down after starting a countdown for all players on the server.";
        ...
        public override List<string> ExecuteCommand(params string[] args)
        {
            ...
            ClientCommander.Instance.Shutdown(duration);

            commandFeedback.AddBroadcastCommand("MicroMade is going down for maintenance.  Please log out soon.");
            return commandFeedback;
        }
    }
}
